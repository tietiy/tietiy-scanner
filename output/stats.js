...
// SHADOW — Funnel section now shows correct label
//        "shadow analysis" when mini scanner is in
//        shadow mode. Previously showed misleading
//        "Mini scanner rejected" count.
// ─────────────────────────────────────────────────────
(function () {
...
function _scoreBar(score, count, maxCount) {
  const width = maxCount
    ? Math.max(
        Math.round((count / maxCount) * 100), 2)
    : 2;
...
function _analyse(raw) {
  ...
  ['UP_TRI', 'DOWN_TRI', 'BULL_PROXY']
    .forEach(function(t) {
      ...
      const typeWins = typeResolved
        .filter(s => OUTCOME_WIN.has(s.outcome));
  ...
  // If rejected records exist but all have same IDs
  // as took records (duplicate pattern = shadow mode
  // was creating both), we detect shadow is ON.
  // More reliable: check if any rejected record has
  // shadow_only field or check rejection counts
  // vs live took counts.
  // Simple heuristic: if rej count > 0 but
  // tookLive count roughly equals rejCount,
  // shadow mode was on for that batch.
  ...
  return `
    <div style="background:#1a1a0a;
      ...
      <div style="color:#8b949e;line-height:1.6;">
        ${d.backfillCount} signals are pre-live
        backfill data (generation=0, before Apr 6).
        These are excluded from win rate calculations
        and Phase 2 decisions. Only live scanner data
        (${d.totalLive} signals from Apr 6+) is used
  ...
  return _section(
    ...
    ${d.hasBackfill
      ? `<div style="font-size:10px;color:#555;
           ...
           (gen=0) tracked but excluded from win rate
         </div>`
      : ''}`);
}
...
function _renderFunnel(d) {
  ...
  // Shadow mode context note
  // In shadow mode, mini scanner passes everything.
  // Rejection log contains "would-be rejections" only —
  // no signals were actually blocked.
  const shadowNote = d.shadowModeActive
    ? `<div style="background:#0d1117;
         ...
         🔍 <b style="color:#8b949e;">
           Shadow mode active
         </b> — mini scanner is observing only.
         No signals are currently being blocked.
         Rejection reasons are logged for future
         analysis. Rules activate one by one as
  ...
  // In shadow mode, the "rejected" count shows
  // would-be rejections, not actual rejections.
  const rejLabel = d.shadowModeActive
    ? `Shadow analysis`
    : `Mini scanner rejected`;
...
function _renderScoreDist(d) {
  ...
  const bars = scores.map(s =>
    _scoreBar(s, d.byScore[s], maxCount)
  ).join('');
