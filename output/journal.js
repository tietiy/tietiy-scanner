...
(function () {
  ...
  function _exitDateStr(sig) {
    ...
    try {
      const d = new Date(sig.exit_date + 'T00:00:00');
      return d.toLocaleDateString('en-IN', {
  ...
  function _badge(sig) {
    ...
    // Resolved outcomes — will show once they come in
    if (outcome === 'TARGET_HIT') {
      const pnl = sig.pnl_pct != null
        ? ` +${parseFloat(sig.pnl_pct).toFixed(1)}%` : '';
      return _badgeHtml('#0d2a0d', '#00C851', `🎯 TARGET${pnl}`);
    }
    if (outcome === 'STOP_HIT') {
      const pnl = sig.pnl_pct != null
        ? ` ${parseFloat(sig.pnl_pct).toFixed(1)}%` : '';
      return _badgeHtml('#2a0a0a', '#f85149', `🛑 STOP${pnl}`);
    }
    if (outcome === 'DAY6_WIN') {
      const pnl = sig.pnl_pct != null
        ? ` +${parseFloat(sig.pnl_pct).toFixed(1)}%` : '';
      return _badgeHtml('#0d2a0d', '#00C851', `✓ DAY6 WIN${pnl}`);
    }
    if (outcome === 'DAY6_LOSS') {
      const pnl = sig.pnl_pct != null
        ? ` ${parseFloat(sig.pnl_pct).toFixed(1)}%` : '';
      return _badgeHtml('#2a0a0a', '#f85149', `✕ DAY6 LOSS${pnl}`);
    }
    if (outcome === 'DAY6_FLAT') {
      return _badgeHtml('#1a1a0a', '#FFD700', '~ FLAT');
    }

    // REJECTED
    if (result === 'REJECTED') {
      return _badgeHtml('#2a0a0a', '#f85149', '✕ REJECTED');
    }

    // Still pending — show day counter
    const day = _dayNum(sig);
    if (day !== null) {
      if (day >= 6) {
        return _badgeHtml('#2a0a0a', '#f85149', '⚠️ EXIT TODAY');
      }
      return _badgeHtml('#1a1a0a', '#FFD700', `Day ${day}/6`);
    }
  ...
  function _card(sig, isRejView) {
    ...
    const age        = sig.age != null ? sig.age : '?';
    ...
    return `
      <div style="background:#161b22;
        ...
        <div style="display:flex;gap:14px;
          ...
          <span>Entry&nbsp;<b style="color:#c9d1d9;">
            ${entry}</b></span>
          <span>Stop&nbsp;<b style="color:#f85149;">
            ${stop}</b></span>
          <span>Target&nbsp;<b style="color:#00C851;">
            ${target}</b></span>
        </div>

        <!-- Row 4: Exit date OR rejection reason -->
        ${!isRejView && exitStr
    ...
    return `
      <div style="display:flex;gap:8px;
        ...
        <button onclick="window._jFilter('took')"
          style="flex:1;${_filter === 'took' ? on : off}
        ...
        <button onclick="window._jFilter('rejected')"
          style="flex:1;${_filter === 'rejected' ? on : off}
            border:1px solid #30363d;
            border-radius:6px;padding:7px 0;
            font-size:12px;cursor:pointer;">
          ✕ Rejected (${rejN})
  ...
  window.renderJournal = function (tietiy) {
    ...
    const entries  = _filter === 'took' ? took : rej;
      ...
      dates.forEach(function (dateStr) {
        ...
        html += `
          <div style="padding:0 16px;">
            ...
            ${group.map(s => _card(s, isRejView)).join('')}
    ...
    // Expose filter toggle globally for onclick
