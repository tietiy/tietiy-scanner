...
def main():
    ...
    # Freshness header — surfaces data staleness at the top so future
    # readers can tell at a glance whether the report covers today's
    # eod_master run or a pre-EOD snapshot.
    gen_ist = datetime.now(ZoneInfo("Asia/Kolkata")).strftime(
        "%Y-%m-%d %H:%M IST")
    outcome_dates = [
        r.get("outcome_date") for r in records
        if isinstance(r.get("outcome_date"), str)
        and r.get("outcome") != "OPEN"
    ]
    last_resolved = max(outcome_dates) if outcome_dates else "n/a"

    sections = [
        f"_Generated: {gen_ist} | Last resolved outcome_date in "
        f"dataset: {last_resolved}_\n",
