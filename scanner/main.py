...
def get_nifty_info():
    try:
        ...
        s_ret     = (2 if ret20>5 else 1 if ret20>2
                     else -2 if ret20<-5 else -1 if ret20<-2 else 0)
...
def get_sector_leaders(sector_momentum):
    return [s for s,m in sector_momentum.items()
            if m == 'Leading']
...
def run_morning_scan():
    ...
    if not status['is_trading']:
        send_holiday_notice(status['next_trading'])
