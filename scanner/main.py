...
BEAR BONUS ACTIVE — UP TRI signals highest conviction
    ...
    if rejected_signals:
        ...
        for s in rejected_signals:
            ...
            reasons = []
            if score < 2:
                reasons.append(f'Score {score}/10 — below min')
            ...
            reason = ' | '.join(reasons)
            rej_rows += f'''<tr>
<td style="color:#555;">{sym}</td>
<td style="color:#555;">{signal} {arrow}</td>
<td style="color:#444;">{score}/10</td>
<td style="color:#444;">{age}</td>
<td style="color:#444;font-size:10px;">{reason}</td>
</tr>'''
        rej_html = f'''
<div style="color:#444;font-size:11px;font-weight:700;
border-left:3px solid #333;padding-left:8px;margin:16px 0 8px;">
...
<table>
<tr><th>Stock</th><th>Signal</th><th>Score</th>
    <th>Age</th><th>Reason</th></tr>
{rej_rows}
</table>
<div style="font-size:10px;color:#444;padding:6px 0;">
Detected but did not meet minimum score threshold.
</div>
</div>'''
    ...
    for t in all_journal[:15]:
        ...
        sc2    = ('#00C851' if status == 'WON' else
                  '#f85149' if status in (
                      'STOPPED', 'CLOSED') else '#FFD700')
        pnl_c  = '#f85149' if str(pnl).startswith(
            '-') else '#00C851'
        jrows += f'''<tr>
...
#tap-overlay{{display:none;position:fixed;top:0;left:0;
              right:0;bottom:0;background:rgba(0,0,0,0.75);
              z-index:100;}}
...
<button class="filter-btn active"
        onclick="filterS('all',event)">All</button>
<button class="filter-btn"
        onclick="filterS('UP_TRI',event)">UP TRI</button>
<button class="filter-btn"
        onclick="filterS('DOWN_TRI',event)">DOWN TRI</button>
<button class="filter-btn"
        onclick="filterS('BULL_PROXY',event)">Proxy</button>
<button class="filter-btn"
        onclick="filterS('age0',event)">Age 0</button>
</div>

<div id="signals">
  <div style="color:#666;text-align:center;
  padding:20px;font-size:12px;">Loading signals...</div>
</div>
...
<div id="tap-panel">
  ...
  gap:8px;margin-bottom:10px;">
    <div class="stat-box">
      ...
      <div id="panel-live" class="stat-val"
      style="color:#58a6ff;">...</div>
    </div>
    <div class="stat-box">
      <div class="stat-label">ENTRY EST</div>
      <div id="panel-entry" class="stat-val"
      style="color:#c9d1d9;">—</div>
    </div>
    <div class="stat-box">
      <div class="stat-label">STOP LOSS</div>
      <div id="panel-stop" class="stat-val"
      style="color:#f85149;">—</div>
    </div>
    <div class="stat-box">
      <div class="stat-label">TARGET</div>
      <div id="panel-target" class="stat-val"
      style="color:#00C851;">—</div>
    </div>
  </div>
  ...
  margin-bottom:10px;font-size:12px;">
    ...
    <span id="panel-pnl-val"
    style="font-weight:700;font-size:14px;"></span>
    <span id="panel-pnl-pct"
    style="color:#666;font-size:11px;margin-left:6px;"></span>
  </div>
  ...
  <div id="panel-status"
  style="font-size:10px;color:#666;text-align:center;
  min-height:16px;"></div>
</div>
  ...
  try {{
    ...
    if (!sigs.length) {{
      el.innerHTML =
        ...
        + 'No signals on ' + latest.date + '</div>';
      return;
    }}
    el.innerHTML = sigs.map(s => {{
      const sym    = (s.stock || s.symbol || '')
                     .replace('.NS','');
      ...
      return `<div class="signal-card"
        ...
        margin-bottom:6px;">
          ...
          <span style="color:${{sc}};font-size:13px;
          font-weight:700;">${{score}}/10</span>
        </div>
...
function openPanel(el) {{
  ...
  document.getElementById('tap-overlay').style.display =
    'block';
...
function toggleRej() {{
  const el  = document.getElementById('rej-section');
