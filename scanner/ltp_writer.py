# ── ltp_writer.py ────────────────────────────────────
# Fetches current LTP for all PENDING signals
# Runs 3 times daily: 11 AM, 1 PM, 3 PM IST
#
# Writes ltp_prices.json to output/
# Frontend cards show live price during market hours
#
# Schema:
#   {
#     "date": "2026-04-07",
#     "fetch_time": "11:02 IST",
#     "prices": {
#       "TATASTEEL.NS": {
#         "ltp":        195.48,
#         "change_pct": +2.67,
#         "fetch_time": "11:02 IST"
#       }
#     }
#   }
# ─────────────────────────────────────────────────────

import os
import sys
import json
from datetime import date, datetime

import yfinance as yf
import pandas as pd

sys.path.insert(0, os.path.dirname(__file__))
from journal import load_history

# ── PATHS ─────────────────────────────────────────────
_HERE   = os.path.dirname(os.path.abspath(__file__))
_ROOT   = os.path.dirname(_HERE)
_OUTPUT = os.path.join(_ROOT, 'output')

LTP_FILE = os.path.join(
    _OUTPUT, 'ltp_prices.json')


# ── FETCH LTP ─────────────────────────────────────────

def _fetch_ltp(symbol, retries=2):
    """
    Fetches current price for a symbol.
    Returns (ltp, prev_close) or (None, None).
    yfinance during market hours returns
    delayed price (~15 min delay) — sufficient
    for monitoring open positions.
    """
    for attempt in range(retries + 1):
        try:
            ticker = yf.Ticker(symbol)
            df = ticker.history(
                period      = '2d',
                interval    = '1d',
                auto_adjust = False,
            )

            if df is None or df.empty:
                continue

            if isinstance(df.columns,
                          pd.MultiIndex):
                df.columns = [
                    c[0] for c in df.columns]

            if df.index.tzinfo:
                df.index = \
                    df.index.tz_localize(None)

            if len(df) >= 2:
                prev_close = float(
                    df['Close'].iloc[-2])
                ltp = float(
                    df['Close'].iloc[-1])
            else:
                prev_close = float(
                    df['Close'].iloc[-1])
                ltp = prev_close

            if ltp > 0:
                return ltp, prev_close

        except Exception as e:
            print(f"[ltp] Fetch error "
                  f"{symbol} "
                  f"attempt {attempt}: {e}")

    return None, None


# ── MAIN FUNCTION ─────────────────────────────────────

def run_ltp_update():
    """
    Main entry. Called from ltp_updater.yml
    at 11 AM, 1 PM, 3 PM IST.

    Only fetches prices for PENDING signals —
    not the full universe.
    Keeps yfinance calls minimal.
    """
    print("[ltp] Starting LTP update...")

    today_str  = date.today().isoformat()
    fetch_time = datetime.utcnow().strftime(
        '%H:%M IST')

    # Load PENDING signals only
    history = load_history()
    pending = [
        s for s in history
        if s.get('result') == 'PENDING'
    ]

    if not pending:
        print("[ltp] No pending signals")
        _write_ltp({}, today_str, fetch_time)
        return

    # Deduplicate symbols
    symbols = list({
        s.get('symbol', '')
        for s in pending
        if s.get('symbol', '')
    })

    print(f"[ltp] Fetching {len(symbols)} "
          f"symbols...")

    prices     = {}
    success    = 0
    failed     = 0

    for symbol in symbols:
        ltp, prev_close = _fetch_ltp(symbol)

        if ltp is None:
            print(f"[ltp] Failed: {symbol}")
            failed += 1
            continue

        change_pct = 0.0
        if prev_close and prev_close > 0:
            change_pct = round(
                (ltp - prev_close) /
                prev_close * 100, 2)

        prices[symbol] = {
            'ltp':        round(ltp, 2),
            'prev_close': round(
                prev_close, 2)
                if prev_close else None,
            'change_pct': change_pct,
            'fetch_time': fetch_time,
        }
        success += 1

        sym_clean = symbol.replace('.NS', '')
        arrow = '▲' if change_pct >= 0 else '▼'
        print(f"[ltp] {sym_clean} "
              f"₹{ltp:.2f} "
              f"{arrow}{abs(change_pct):.1f}%")

    _write_ltp(prices, today_str, fetch_time)

    print(f"[ltp] Complete — "
          f"success:{success} "
          f"failed:{failed}")


# ── WRITE ltp_prices.json ─────────────────────────────

def _write_ltp(prices, today_str, fetch_time):
    os.makedirs(_OUTPUT, exist_ok=True)

    data = {
        'date':       today_str,
        'fetch_time': fetch_time,
        'count':      len(prices),
        'prices':     prices,
    }

    try:
        with open(LTP_FILE, 'w') as f:
            json.dump(
                data, f, indent=2, default=str)
        print(f"[ltp] ltp_prices.json written "
              f"→ {len(prices)} prices")
    except Exception as e:
        print(f"[ltp] Write failed: {e}")


# ── ENTRY POINT ───────────────────────────────────────

if __name__ == '__main__':
    run_ltp_update()
